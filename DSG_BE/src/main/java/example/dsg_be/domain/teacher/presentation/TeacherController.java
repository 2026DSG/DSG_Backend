...
public class TeacherController {
    ...
    public void uploadExcel(@RequestParam("file") MultipartFile file) {
        teacherExcelUploadService.excute(file);
    }

    @PutMapping("/excel")
    @ResponseStatus(HttpStatus.OK)
    public void updateExcel(@RequestParam("file") MultipartFile file) {
        teacherExcelUpdateService.excute(file);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public void addTeacher(@RequestBody TeacherRequest teacherRequest) {
        teacherAddService.excute(teacherRequest);
    }

    @GetMapping
    @ResponseStatus(HttpStatus.OK)
    public List<TeacherResponse> getTeacherList() {
        return teacherListReadService.excute();
    }

    @DeleteMapping("/{teacher-id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteTeacher(@PathVariable("teacher-id") Long id) {
        teacherDeleteService.excute(id);
    }

    @GetMapping("/excel")
    public ResponseEntity<Resource> downloadaExcel() throws IOException {
        byte[] content = teacherExcelDownloadService.excute(); // 서비스에서 받아온 값을
